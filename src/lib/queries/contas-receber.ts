...
export async function getContasReceber(
  ...
  return cacheList(
    CACHE_KEYS.list("contas-receber", page, pageSize, search || ""),
    () => _fetchContasReceber(page, pageSize, search)
  );
}

export function prefetchNextPage(page: number, pageSize: number, search?: string) {
  void cacheList(
    CACHE_KEYS.list("contas-receber", page + 1, pageSize, search || ""),
    () => _fetchContasReceber(page + 1, pageSize, search)
  );
}

async function _fetchContasReceber(
  page = 1,
  pageSize = 50,
  search?: string
): Promise<ContasReceberResult> {
